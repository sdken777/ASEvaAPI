...
namespace ExampleLibrary
{
    ...
    new LinearAxis() { Key = "x_axis", AbsoluteMinimum = x0, AbsoluteMaximum = x1, Position = AxisPosition.Top });
        ...
        public static PlotModel ContourSeriesProduceFakeConnections()
        {
            ...
            using (var stream = typeof(Issues).GetTypeInfo().Assembly.GetManifestResourceStream("DodgyContourData.tsv"))
            {
            ...
            using (var stream = typeof(Issues).GetTypeInfo().Assembly.GetManifestResourceStream("X.txt"))
            {
                using (var reader = new System.IO.StreamReader(stream))
                {
            ...
            using (var stream = typeof(Issues).GetTypeInfo().Assembly.GetManifestResourceStream("Y.txt"))
            {
                using (var reader = new System.IO.StreamReader(stream))
                {
