...
namespace ExampleLibrary
{
    ...
    {
        public static float PixelScale { private get; set; }
        ...
        public static PlotModel DrawTextColors()
        {
            ...
            model.Annotations.Add(new DelegateAnnotation(rc =>
            {
                ...
                double D = FontSize * 1.6 * PixelScale;
                double X = 20 * PixelScale;
                double y = 20 * PixelScale - D;
                ...
                rc.FillRectangle(new OxyRect(X, y + D + 15 * PixelScale, 200 * PixelScale, 10 * PixelScale), OxyColors.Black, EdgeRenderingMode.Adaptive);
        ...
        public static PlotModel DrawTextFontSizes()
        {
            ...
            {
                double X = 20 * PixelScale;
                double y = 20 * PixelScale;

                // Font sizes
                foreach (var size in new[] { 10, 16, 24, 36, 48 })
                {
                    rc.DrawText(new ScreenPoint(X, y), size + "pt", OxyColors.Black, "Arial", size);
                    rc.DrawText(new ScreenPoint(X + 200 * PixelScale, y), size + "pt", OxyColors.Black, "Arial", size, FontWeights.Bold);
                    y += size * 1.6 * PixelScale;
                }
            }));
            return model;
        }
        ...
        public static PlotModel DrawTextRotation()
        {
            ...
            model.Annotations.Add(new DelegateAnnotation(rc =>
            {
                var origin = new ScreenPoint(200 * PixelScale, 200 * PixelScale);
                rc.FillCircle(origin, 3 * PixelScale, OxyColors.Blue, EdgeRenderingMode.Adaptive);
        ...
        public static PlotModel DrawTextAlignment()
        {
            ...
            model.Annotations.Add(new DelegateAnnotation(rc =>
            {
                ...
                for (var ha = HorizontalAlignment.Left; ha <= HorizontalAlignment.Right; ha++)
                {
                    ...
                    {
                        var origin = new ScreenPoint(((((int)ha + 1) * 200) + 20) * PixelScale, ((((int)va + 1) * FontSize * 3) + 20) * PixelScale);
                        rc.FillCircle(origin, 3 * PixelScale, OxyColors.Blue, EdgeRenderingMode.Adaptive);
        ...
        public static PlotModel DrawTextMultiLineAlignment()
        {
            ...
            model.Annotations.Add(new DelegateAnnotation(rc =>
            {
                ...
                for (var ha = HorizontalAlignment.Left; ha <= HorizontalAlignment.Right; ha++)
                {
                    for (var va = VerticalAlignment.Top; va <= VerticalAlignment.Bottom; va++)
                    {
                        var origin = new ScreenPoint(((((int)ha + 1) * 200) + 20) * PixelScale, ((((int)va + 1) * FontSize * 6) + 20) * PixelScale);
                        rc.FillCircle(origin, 3 * PixelScale, OxyColors.Blue, EdgeRenderingMode.Adaptive);
        ...
        public static PlotModel MathTextRotation()
        {
            ...
            {
                var origin = new ScreenPoint(200 * PixelScale, 200 * PixelScale);
                var origin2 = new ScreenPoint(400 * PixelScale, 200 * PixelScale);
                rc.FillCircle(origin, 3 * PixelScale, OxyColors.Blue, EdgeRenderingMode.Adaptive);
                for (int rotation = 0; rotation < 360; rotation += 45)
                {
                    var text = "     A_{2}^{3}B";
        ...
        public static PlotModel DrawTextAlignmentRotation()
        {
            ...
            model.Annotations.Add(new DelegateAnnotation(rc =>
            {
                for (var ha = HorizontalAlignment.Left; ha <= HorizontalAlignment.Right; ha++)
                {
                    for (var va = VerticalAlignment.Top; va <= VerticalAlignment.Bottom; va++)
                    {
                        var origin = new ScreenPoint((((int)ha + 2) * 130) * PixelScale, (((int)va + 2) * 130) * PixelScale);
                        rc.FillCircle(origin, 3 * PixelScale, OxyColors.Blue, EdgeRenderingMode.Adaptive);
        ...
        public static PlotModel DrawMultilineTextAlignmentRotation()
        {
            ...
            model.Annotations.Add(new DelegateAnnotation(rc =>
            {
                for (var ha = HorizontalAlignment.Left; ha <= HorizontalAlignment.Right; ha++)
                {
                    for (var va = VerticalAlignment.Top; va <= VerticalAlignment.Bottom; va++)
                    {
                        var origin = new ScreenPoint((((int)ha + 2) * 170) * PixelScale, (((int)va + 2) * 170) * PixelScale);
                        rc.FillCircle(origin, 3 * PixelScale, OxyColors.Blue, EdgeRenderingMode.Adaptive);
            ...
            model.Annotations.Add(new DelegateAnnotation(rc =>
            {
                ...
                foreach (double fontSize in fontSizes)
                {
                    ...
                    foreach (var s in strings)
                    {
                        ...
                        y += size.Height + 20 * PixelScale;
                    }

                    x += maxWidth + 20 * PixelScale;
                }
            }));
            return model;
        }
            ...
            model.Annotations.Add(
                new DelegateAnnotation(
                    rc =>
                    {
                        ...
                        var expectedSize = new OxySize(expectedWidth * PixelScale, expectedHeight * PixelScale);
                        rc.DrawText(new ScreenPoint(300 * PixelScale, 50 * PixelScale), "Font size: " + fontSize, OxyColors.Black, font, 12);
                        rc.DrawText(new ScreenPoint(300 * PixelScale, 70 * PixelScale), "Actual size: " + size.ToString("0.00", CultureInfo.InvariantCulture), OxyColors.Black, font, 12);
                        rc.DrawText(new ScreenPoint(300 * PixelScale, 90 * PixelScale), "Size on " + platform + ": " + expectedSize.ToString("0.00", CultureInfo.InvariantCulture), OxyColors.Green, font, 12);

                        var p = new ScreenPoint(20 * PixelScale, 50 * PixelScale);
                        rc.DrawText(p, text, OxyColors.Black, font, fontSize);

                        rc.FillCircle(p, 3 * PixelScale, OxyColors.Black, EdgeRenderingMode.Adaptive);
                        ...
                        var x1 = p.X - 10 * PixelScale;
                        var x2 = p.X + expectedSize.Width + 10 * PixelScale;
                        ...
                        var y1 = p.Y - 10 * PixelScale;
                        var y2 = p.Y + expectedSize.Height + 10 * PixelScale;
        ...
        public static PlotModel Clipping()
        {
            ...
            model.Annotations.Add(new DelegateAnnotation(rc =>
            {
                ...
                var currentLine = 20d * PixelScale;
                double lineHeight = 60 * PixelScale;
                double clipRectSize = 40 * PixelScale;
                double clipRectMargin = 20 * PixelScale;
                double testCaseMargin = 20 * PixelScale;
                double descriptionMargin = 200 * PixelScale;
        ...
        public static PlotModel Rectangles()
        {
            ...
            model.Annotations.Add(new DelegateAnnotation(rc =>
            {
                for (int i = 0; i < THICKNESS_STEPS; i++)
                {
                    ...
                    rc.DrawText(new ScreenPoint(left * PixelScale, OFFSET_TOP / 2 * PixelScale), strokeThickness.ToString(), OxyColors.Black, horizontalAlignment: HorizontalAlignment.Center, verticalAlignment: VerticalAlignment.Middle);
                }
                ...
                foreach (EdgeRenderingMode edgeRenderingMode in Enum.GetValues(typeof(EdgeRenderingMode)))
                {
                    ...
                    rc.DrawText(new ScreenPoint(10 * PixelScale, (top + 10) * PixelScale), edgeRenderingMode.ToString(), OxyColors.Black, verticalAlignment: VerticalAlignment.Middle);
                    for (int i = 0; i < THICKNESS_STEPS; i++)
                    {
                        var left = OFFSET_LEFT + i * GRID_SIZE;
                        var rect = new OxyRect(left * PixelScale, top * PixelScale, TILE_SIZE * PixelScale, TILE_SIZE * PixelScale);
                        var strokeThickness = i * THICKNESS_STEP;
        ...
        public static PlotModel Lines()
        {
            ...
            model.Annotations.Add(new DelegateAnnotation(rc =>
            {
                ...
                foreach (EdgeRenderingMode edgeRenderingMode in Enum.GetValues(typeof(EdgeRenderingMode)))
                {
                    ...
                    {
                        var left = OFFSET_LEFT + i * GRID_SIZE;
                        var topLeft = new ScreenPoint(left * PixelScale, top * PixelScale);
                        var bottomLeft = new ScreenPoint(left * PixelScale, (top + TILE_SIZE) * PixelScale);
                        var topRight = new ScreenPoint((left + TILE_SIZE) * PixelScale, top * PixelScale);
                        var bottomRight = new ScreenPoint((left + TILE_SIZE) * PixelScale, (top + TILE_SIZE) * PixelScale);
                        var middleLeft = new ScreenPoint(left * PixelScale, (top + TILE_SIZE / 2) * PixelScale);
                        var strokeThickness = i * THICKNESS_STEP;
            ...
            model.Annotations.Add(new DelegateAnnotation(rc =>
            {
                ...
                foreach (EdgeRenderingMode edgeRenderingMode in Enum.GetValues(typeof(EdgeRenderingMode)))
                {
                    ...
                    for (int i = 0; i < THICKNESS_STEPS; i++)
                    {
                        ...
                        var points = new []
                        {
                            new ScreenPoint((left + TILE_SIZE * .4) * PixelScale, top * PixelScale),
                            new ScreenPoint((left + TILE_SIZE) * PixelScale, (top + TILE_SIZE * .2) * PixelScale),
                            new ScreenPoint((left + TILE_SIZE * .9) * PixelScale, (top + TILE_SIZE * .8) * PixelScale),
                            new ScreenPoint((left + TILE_SIZE * .5) * PixelScale, (top + TILE_SIZE) * PixelScale),
                            new ScreenPoint(left * PixelScale, (top + TILE_SIZE * .6) * PixelScale),
                        };
        ...
        public static PlotModel Ellipses()
        {
            ...
            model.Annotations.Add(new DelegateAnnotation(rc =>
            {
                ...
                foreach (EdgeRenderingMode edgeRenderingMode in Enum.GetValues(typeof(EdgeRenderingMode)))
                {
                    ...
                    {
                        var left = OFFSET_LEFT + i * GRID_SIZE;
                        var rect = new OxyRect(left * PixelScale, (top + TILE_SIZE * .1) * PixelScale, TILE_SIZE * PixelScale, (TILE_SIZE * .8) * PixelScale);
                        var strokeThickness = i * THICKNESS_STEP;
        ...
        {
            double STROKE_THICKNESS = 15 * PixelScale;
            double LINE_LENGTH = 60 * PixelScale;
            var ANGLES = new[] { 135, 90, 45, 22.5 };
            double COL_WIDTH = 140 * PixelScale;
            double ROW_HEIGHT = 90 * PixelScale;
            double ROW_HEADER_WIDTH = 50 * PixelScale;
            double COL_HEADER_HEIGHT = 50 * PixelScale;
        ...
        public static PlotModel EllipseDrawing()
        {
            ...
            model.Annotations.Add(new DelegateAnnotation(rc =>
            {
                var rect = new OxyRect((CENTER_X - RADIUS_X) * PixelScale, (CENTER_Y - RADIUS_Y) * PixelScale, RADIUS_X * 2 * PixelScale, RADIUS_Y * 2 * PixelScale);
                ...
                rc.DrawText(new ScreenPoint(CENTER_X * PixelScale, CENTER_Y * PixelScale), "The white ellipse (drawn by Renderer) should match the black ellipse (drawn as Path).", OxyColors.Black, fontSize: 12, horizontalAlignment: HorizontalAlignment.Center, verticalAlignment: VerticalAlignment.Middle);
            }));
            return model;
        }
